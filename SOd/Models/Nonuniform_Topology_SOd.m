...
function[model_out]=Nonuniform_Topology_SOd(n,d,p,p_node_crpt,p_edge_crpt, sigma_in, sigma_out, crpt_type)
    ...
    crptInd = false(1,m);
    R_crpt = zeros(d,d,n); % only for self-consistent corruption
    ...
    RijMat(:,:,noiseInd)= ...
    RijMat(:,:,noiseInd)+sigma_in*randn(d,d,sum(noiseInd)); % add noise

    RijMat(:,:,crptInd)= ...
    RijMat(:,:,crptInd)+sigma_out*randn(d,d,sum(crptInd)); % add noise
