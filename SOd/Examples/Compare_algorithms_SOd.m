...
% n=100; d=5; p=0.5; q=0.6; sigma=0; crpt_type='uniform';
...
n=100; d=5; p=0.5; p_node_crpt=0.5; p_edge_crpt=0.75; 
sigma_in=0; sigma_out=1; crpt_type='self-consistent';
model_out = Nonuniform_Topology_SOd(n,d,p,p_node_crpt,p_edge_crpt, sigma_in, sigma_out, crpt_type);
...
% run Spectral
R_SP = Spectral_SOd(Ind, RijMat);

% run SDP  
R_SDP = SDP_SOd(Ind, RijMat); %commented this out if cvx is not installed

% run IRLS
niter = 100;
R_IRLS = IRLS_SOd(Ind, RijMat, niter);

% run CEMP+MST
R_CEMP_MST = CEMP_MST_SOd(Ind, RijMat, parameters);

% run CEMP+GCW
R_CEMP_GCW = CEMP_GCW_SOd(Ind, RijMat, parameters);

[mean_error_SP] = evaluate_error_SOd(R_SP, Rij_orig, Ind);
[mean_error_SDP] = evaluate_error_SOd(R_SDP, Rij_orig, Ind);
[mean_error_IRLS] = evaluate_error_SOd(R_IRLS, Rij_orig, Ind);
[mean_error_CEMP_MST] = evaluate_error_SOd(R_CEMP_MST, Rij_orig, Ind);
[mean_error_CEMP_GCW] = evaluate_error_SOd(R_CEMP_GCW, Rij_orig, Ind);
