...
function[SVec] = CEMP_Z2(Ind,zij,parameters)
    %CEMP parameters
    ...
    for l = 1:m_pos
        ...
        jkvec((cum_ind(l)+1):cum_ind(l+1)) =  zijMat(j,CoInd_ij);
        kivec((cum_ind(l)+1):cum_ind(l+1)) =  zijMat(CoInd_ij,i);
    end

    ijvec = zij(Ind_ij);


    z_cycle = ijvec.*jkvec.*kivec;
    S0_long = 0.5*(1-z_cycle);
    S0_vec = ones(1,m);
